...
static int16_t cout_p = 0, cout_y = 0,cout_m;
static int16_t new_data_p = 4096, old_data_p,new_data_m;
static int16_t new_data_y = 4096, old_data_y,old_data_m;
...
int encoder_value_for_fire(int16_t input_data)
{
	old_data_m = new_data_m;
	new_data_m = input_data;
	
	if((new_data_m - old_data_m) < -5000)
		cout_m++;
	else if((new_data_m - old_data_m) > 5000)
		cout_m--;
...
void ReceiveCAN1Data(MyData *mydata)
{
	...
	{	
		...
		case CAN_BUS1_MOTOR7:
		{
			...
			CAN1_Motor_PID[6].position_get=encoder_value_for_fire((rx_message.Data[0] << 8) | rx_message.Data[1]);
...
void CAN1_Send_Motor_data(CAN_TypeDef *CANx, int16_t cm1_iq, int16_t cm2_iq, int16_t cm3_iq, int16_t cm4_iq)
{
    CanTxMsg tx_message;
