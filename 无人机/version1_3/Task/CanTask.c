...
void ReceiveCAN1Data(MyData *mydata)
{
	...
	{	
		...
		//	mydata->usemotor.rm6623_p.encoder = encoder_value_for_pitch(origion_pith_encoder_val);//码盘值连续化处理
			...
			CAN1_Motor_PID[5].position_get = (encoder_value_for_pitch((rx_message.Data[0] << 8) | rx_message.Data[1]))/ 36;	
      if(init_flag == 0)	{
			  init_flag = 1;  CAN1_Motor_PID[5].init_position = CAN1_Motor_PID[5].position_get;
			}		
		}
		break;
