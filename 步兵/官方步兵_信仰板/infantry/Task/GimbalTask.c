  ...
  if(Control_Mode == 0)  
	...
	else
	{
	  YawControl->ControlData = -YawControl->Angle ;
		...
		PositionOutPut = (AutoData.YawAxiaAngle ) * 8.2f;
		else
...
void PitchControlCalc(GIMBALCONTORL *PitchControl) //计算
{
	...
	else
	{
	   PitchControl->ControlData =  Encoder_init- PitchControl->Encoder;
		...
		{
		  PositionOutPut = (AutoData.PitchAxiaAngle + 7.5) * 8.0f;
		}
		else
		{
		   PositionOutPut = 0;
	   ...
	   VelocityOutPut = (PositionOutPut - PitchControl->Gyro) * 40.0f;
	}
...
{
   return(( PitchControl.Encoder	- PitchControl.Encoder_Init ) *360 /8192 ) * 4; 	//初始化的机械角
}
