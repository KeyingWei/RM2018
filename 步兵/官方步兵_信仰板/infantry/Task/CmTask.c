...
void Fire_Motor_Pidparams_init()
{
 ...
 for(i=0;i<4;i++)
	...
	PID_struct_init(&CAN1_Motor_PID[6],DELTA_PID,7000,500,15,2.5f,0.05); 
}
...
void CM_Motor_Pidparams_init()
{
  int i =0;
	for(i=0;i<4;i++)
	{
	  PID_struct_init(&CAN1_Motor_PID[i],DELTA_PID,7000,500,0,0.3f,0.01f);
	}
}
...
void CMControlCalc(char cm_mode)
{
	...
	else //初始化完成后根据Y轴电机的机械角得到底盘相对云台的角度值
	{
		...
		{
	     CmCotrol.Angle = (CmCotrol.Encoder_Init - CmCotrol.Encoder) * 360 / 8192;	
			 CmCotrol.Control_CH2 = CmCotrol.Angle * 250;
		}
	  else	if(cm_mode == 1)
		{
			  if(TurnLeftFlag == 0)
