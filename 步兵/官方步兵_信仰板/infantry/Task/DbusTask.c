...
static void MouseKeyboardControlPress(MyData *mydata)
{
	...
	mydata->dbus.mouse.x = constrain(mydata->dbus.mouse.x,-35,35);
	mydata->dbus.mouse.y = constrain(mydata->dbus.mouse.y,-30,30);
	
   mydata->Cortol_ch2 -= mydata->dbus.mouse.x   *  0.09f;
	 ...
	 switch(mydata->dbus.key.v)
	 {
	    ...
	    if(mydata->dbus.key.v == KEY_PRESSED_OFFSET_W || mydata->dbus.key.v == KEY_PRESSED_OFFSET_S)
			{
												...
												switch(Referee_date1.GameRobotState_t.robotLevel)
												{
												  case 1:
													...
													case 3:
																	Set_Fire_Fre(2200);
																	Set_fire_speed(4500);		
																	ShootMotor_Left = ShootMotor_Right = 128;																
														break;
													default:
																	Set_Fire_Fre(2200);
																	Set_fire_speed(4500);		
