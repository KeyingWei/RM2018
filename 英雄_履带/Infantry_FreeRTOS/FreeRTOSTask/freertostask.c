...
Motor_SpeedLoopData_t CM_Motor[6];   
		...
		xEventGroupClearBits(OfflineDetectedEnt,(1<<1));
		  ...
		  switch(RemotData.rc.s2)
		  {
		      ...
		      case RC_SW_MID:
				  {
					  ...
					  if(RemotData.rc.s1 == RC_SW_UP)//履带控制
					  {
					      VALVE = 0;
						  xEventGroupClearBits(CmGimbalOutputEnt,0x02);	
					  }
					  else if(RemotData.rc.s1 == RC_SW_MID) //底盘控制
					  {
						   VALVE = 1;
		      ...
		      case RC_SW_UP:
			  {
                      ...
                      xEventGroupClearBits(CmGimbalOutputEnt,0x01);			  
			  }
				  break;
			  default :
				  break;
		...
		switch(rx_message.StdId)
			{
				...
				case 0x204:
					...
					break;
				case 0x205:					
					CM_Motor[4].getspeed = (rx_message.Data[2]<<8) | rx_message.Data[3];
				    CM_Motor[4].onlinecnt = 0;
					xEventGroupClearBits(OfflineDetectedEnt,(1<<2));
					break;
				case 0x206:
					CM_Motor[5].getspeed = (rx_message.Data[2]<<8) | rx_message.Data[3];
				    CM_Motor[5].onlinecnt = 0;
   ...
   for(;;)
	{
		...
		if((ControlBits & 0x01)  == 1)
		{
			if((ControlBits & 0x02)  == 0x2)
			{
				...
				CM_Motor[0].setSpeed = - CM_Motor[0].setSpeed;
				CM_Motor[1].setSpeed = - CM_Motor[1].setSpeed;
				CM_Motor[3].setSpeed = - CM_Motor[3].setSpeed;
			
			    CM_Motor[4].setSpeed = 0;
		        CM_Motor[5].setSpeed = 0;
			}
			else
			{
				 CM_Motor[4].setSpeed = controlvalue.forwardOrbackward - \
										 controlvalue.left_right	    ;
										 	
				 CM_Motor[5].setSpeed = controlvalue.forwardOrbackward + \
										 controlvalue.left_right	  ;
					
					CM_Motor[5].setSpeed = -CM_Motor[5].setSpeed;	
  ...
  for(;;)
   {
	   ...
	   cmspeed[4] = -cmspeed[4];
	  if((ControlBits & 0x1) == 0x1)
	  {
		  CanSendMess(CAN1,SEND_ID205_207,&cmspeed[4]); 
